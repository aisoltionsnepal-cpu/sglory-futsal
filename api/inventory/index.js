  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      const r = await getPool().query(
        'UPDATE inventory SET item_name=COALESCE($1,item_name),item_type=COALESCE($2,item_type),brand=COALESCE($3,brand),price_per_piece=COALESCE($4,price_per_piece),pack_size=COALESCE($5,pack_size),stock_in_pieces=COALESCE($6,stock_in_pieces),updated_at=CURRENT_TIMESTAMP WHERE id=$7 RETURNING *',
        [itemName, itemType, brand, pricePerPiece, packSize, stockInPieces, id]
      );
